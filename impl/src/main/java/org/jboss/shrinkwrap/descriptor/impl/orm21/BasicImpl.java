...
public class BasicImpl<T> implements Child<T>, Basic<T>
{
   ...
   // --------------------------------------------------------------------------------------------------------||
   // ClassName: Basic ElementName: orm:convert ElementType : convert
   // MaxOccurs: -  isGeneric: true   isAttribute: false isEnum: false isDataType: false
   // --------------------------------------------------------------------------------------------------------||

   /**
    * If not already created, a new <code>convert</code> element with the given value will be created.
    * Otherwise, the existing <code>convert</code> element will be returned.
    * @return  a new or existing instance of <code>Convert<Basic<T>></code> 
    */
   public Convert<Basic<T>> getOrCreateConvert()
   {
      Node node = childNode.getOrCreate("convert");
      Convert<Basic<T>> convert = new ConvertImpl<Basic<T>>(this, "convert", childNode, node);
      return convert;
   }

   /**
    * Removes the <code>convert</code> element 
    * @return the current instance of <code>Basic<T></code> 
    */
   public Basic<T> removeConvert()
   {
